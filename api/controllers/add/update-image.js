...
const checkAndUpdate = (inputs, exits) => {
  cloudinary.v2.api.resources(
    { public_ids: inputs.publicId, max_results: 1 },
    (error, result) => {
      if (error) return exits.error(error)
      if (
        result.resources.length === 1 &&
        result.resources[0].public_id === inputs.publicId
      ) {
        Add.update(inputs.id, { photoPublicId: inputs.publicId })
          .meta({ fetch: true })
          .exec((err, updated) => {
...
module.exports = {
  ...
  description:
    'Checks if the publicId exists on cloudinary and updates the photoPublicId accordingly',
