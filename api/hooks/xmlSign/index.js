...
var fs = require('fs')
  ...
  , pem=require('pem');
require('date-utils');
var password = sails.config.custom.sriPassword;
var signingKey;
...
module.exports = (sails)=>{
  var setUp = (next)=>{
   var p = path.resolve(__dirname, '../../../certificates/ehr.p12')
   pem.readPkcs12(p,{p12Password:password} ,(err,cert)=>{
    ...
    //  certificate=cert.cert.split('-----')[2];
     certificate=cert.cert.split('-----')[2];
     signingKey=ursa.createPrivateKey(cert.key);
     pem.getModulus(cert.key,(err,mod)=>{
       if(err)return next(err);
       modulus=Buffer.from('00'+mod.modulus).toString('base64');
